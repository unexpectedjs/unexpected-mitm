...
module.exports = {
    ...
    installInto: function (expect) {
        ...
        expectForRendering.output.indentationWidth = 4;
        function stringify(obj) {
            return expectForRendering.inspect(obj).toString('text');
        }

        function injectRecordedExchanges(sourceText, recordedExchanges, pos) {
            while (sourceText.charAt(pos - 1) !== '\n') {
                pos -= 1;
            }
            var indentationUnit = detectIndent(sourceText);
            if (indentationUnit) {
                indentationUnit = indentationUnit.indent;
            } else {
                indentationUnit = '    ';
            }
            var searchRegExp = /( *)([^ ]*)(['"])with http recorded\3,/g;
            searchRegExp.lastIndex = pos;
            return sourceText.replace(searchRegExp, function ($0, lineIndentation, before1, quote) {
                return lineIndentation + quote + 'with mocked out http' + quote + ', ' + stringify(recordedExchanges).replace(/\n^/mg, '\n' + lineIndentation + indentationUnit + indentationUnit) + ',';
            });
        }
                ...
                this.args.push(function (err) {
                    ...
                    if (!callbackCalled) {
                        ...
                        setImmediate(function () {
                            ...
                            console.log('recordedExchanges', stringify(recordedExchanges));

                            var containingCallsite = stack.filter(function (parentCallsite) {
                                return parentCallsite.receiver.file && /\.js$/.test(parentCallsite.receiver.file);
                            }).shift();
                            if (containingCallsite.receiver.file) {
                                var containingFileText = fs.readFileSync(containingCallsite.receiver.file, 'utf-8');
                                console.log(injectRecordedExchanges(containingFileText, recordedExchanges, containingCallsite.pos));
                                console.log(containingCallsite);
                            }
