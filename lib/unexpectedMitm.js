  ...
  installInto(expect) {
    ...
    function stringify(obj, indentationWidth, rendererOverride) {
      ...
      const output = Array.isArray(obj)
        ? obj.map(formatRecordedExchange)
        : formatRecordedExchange(obj);
      return renderer.inspect(output, Infinity).toString('text');
    }
    ...
    function recordPendingInjection(injectionCallsite, recordedExchanges) {
      ...
      if (matchSearchRegExp) {
        ...
        (injectionsBySourceFileName[sourceFileName] =
          ...
          replacement: `${
            lineIndentation + before + quote
          }with http mocked out${quote}, ${stringify(
            recordedExchanges,
            indentationWidth
          ).replace(/\n^/gm, `\n${lineIndentation}`)}${
            after === ' ' ? `, ${quote}` : ','
