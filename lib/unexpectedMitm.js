...
module.exports = {
  ...
  installInto(expect) {
    ...
    const getSourceText = memoizeSync(sourceFileName => fs.readFileSync(sourceFileName, 'utf-8'));
      ...
      if (matchSearchRegExp) {
        ...
        (injectionsBySourceFileName[sourceFileName] =
          ...
          replacement:
            `${lineIndentation +
before +
quote}with http mocked out${quote}, ${stringify(recordedExchanges, indentationWidth).replace(
  /\n^/gm,
  `\n${lineIndentation}`
)}${after === ' ' ? `, ${quote}` : ','}`
        });
        ...
        console.warn(
          `unexpected-mitm: Could not find the right place to inject the recorded exchanges into ${sourceFileName} (around line ${sourceLineNumber}): ${stringify(recordedExchanges, indentationWidth, expect)}`
        );
      }
    }
