...
class UnexpectedMitmMocker {
  constructor(options) {
    ...
    if (options.strategy) {
      if (!Array.isArray(options.strategy.requestDescriptions)) {
        throw new Error(
  ...
  get requestDescriptions() {
    return this.strategy.requestDescriptions;
  }
    ...
    return new Promise((resolve, reject) => {
      mitm
        .on('connect', (socket, opts) => {
          // Store the client socket and connect options so they can be read in the request handler:
          // https://github.com/moll/node-mitm/issues/14#issuecomment-437388337
          socket._handle.remote._mitm = {
            clientSocket: socket,
