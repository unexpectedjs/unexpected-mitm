  ...
  return expect
    ...
    .then(() => {
      if (
        mockResponse &&
        !mockResponseError &&
        (Array.isArray(mockResponse.body) ||
          (mockResponse.body &&
            typeof mockResponse.body === 'object' &&
            (typeof Buffer === 'undefined' ||
              !Buffer.isBuffer(mockResponse.body))))
      ) {
...
function attachResponseHooks(res, callback) {
  ...
  const connection = NODE_10_AND_LATER ? res.connection._handle : res.connection;
  ...
  return new Promise((resolve, reject) => {
    mitm.on(
      ...
      createSerializedRequestHandler((req, res) => {
        ...
        const hasRequestDescription = !!currentDescription;
        ...
        const requestDescription = currentDescription;
          ...
          .then(result => {
            ...
            if (typeof responseProperties === 'function') {
              ...
              return Promise.resolve()
                ...
                .catch(err => // ensure delivery of the caught error to the underlying socket
              ({
                response: null,
                error: err
              }));
